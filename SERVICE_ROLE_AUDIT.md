...
| `apps/platform/src/app/(app)/customers/[id]/flow/actions.ts` | Server Action `updateFlowSettingsAction` que persiste `arrival_radius_meters` en `customers.flow_engine_overrides`. Tras `requireRole('admin', 'dispatcher')` valida el customer destino vía `createServerClient` (sesión normal con RLS — el user solo lee SU customer). Usa `service_role` solo para el UPDATE final porque el shell del platform aún no modela "operator subclients" y necesita escribir el override del único customer activo. Defensa en profundidad: status='active' filter + el caller ya validado. |
